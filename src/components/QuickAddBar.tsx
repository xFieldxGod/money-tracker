  ...
  return (
    <div className="bg-white border border-slate-200/45 p-3 rounded-[24px] shadow-premium">
      <form onSubmit={handleSubmit} className="flex items-center gap-2">
        <span className="text-lg pl-1.5">✨</span>
        <input
          type="text"
          value={text}
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-all shadow-sm shadow-indigo-100 active:scale-95 disabled:opacity-50 cursor-pointer whitespace-nowrap"
        >
          {loading ? '🤖 วิเคราะห์...' : 'บันทึกด้วย AI'}
        </button>
      </form>
      {error && (
        <p className="text-[11px] font-semibold text-amber-600 mt-2 pl-1.5">⚠️ {error}</p>
      )}
    </div>
  )
}
